...
graph=Graph()
rootPath = "/home/ytwen/observationData_follower_one/CA"
counter=0
timeIntervalAll=[]
rawIntervalAll=[]
distanceAll=[]
...
for user in users:
    ...
    for friend in friends:
        ...
        for record in visitRecords:
            ...
            userVisitRecords = graph.cypher.execute("MATCH (n:CAUser {id:{userid}})-[r:VISITED]->(p:CAPlace {id:{pid}}) RETURN r.atTime ORDER BY r.atTime;",{"userid":userID,"pid":record['p.id']})
            #userVisitRecords = graph.cypher.execute("MATCH (n:CAUser {id:{userid}})-[r:VISITED]->(p:CAPlace {id:{pid}}) RETURN r.atTime ORDER BY r.atTime AESC;",{"userid":userID,"pid":record['p.id']})
    ...
    f = open('/home/ytwen/observationData_follower_one/CA/'+userID+'.csv','wb')
    ...
    w = open('/home/ytwen/rawobservationData_follower_one/CA/'+userID,'wb')
    w.write(json.dumps(rawInterval))
    w.close()
    
    d = open('/home/ytwen/observationData_follower_one/CA/'+userID+'_distance','wb')
    d.write(json.dumps(distance))
    d.close()

    
z = open('/home/ytwen/observationData_follower_one/CA/sumData.csv','wb')
...
y = open('/home/ytwen/observationData_follower_one/CA/sumData_distance','wb')

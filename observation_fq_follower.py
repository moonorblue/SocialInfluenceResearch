...
graph=Graph()
rootPath = "/home/ytwen/observationData_follower_one/fs"
counter=0
timeIntervalAll=[]
rawIntervalAll=[]
distanceAll=[]
...
users = graph.cypher.execute("MATCH (n:FSLUser) RETURN n.id")
for user in users:
...
#                        timeIntervalAll[366] = timeIntervalAll[366] + 1
    ...
    f = open('/home/ytwen/observationData_follower_one/fs/'+userID+'.csv','wb')
    for i in xrange(len(timeInterval)):
        f.write(str(i)+','+str(timeInterval[i])+','+str(json.dumps(distanceWithDay[i]))+'\n')
    f.close()

    w = open('/home/ytwen/rawobservationData_follower_one/fs/'+userID,'wb')
    w.write(json.dumps(rawInterval))
    w.close()
    
    d = open('/home/ytwen/observationData_follower_one/fs/'+userID+'_distance','wb')
...
z = open('/home/ytwen/observationData_follower_one/fs/sumData.csv','wb')
for i in xrange(len(timeIntervalAll)):
    z.write(str(i)+','+str(timeIntervalAll[i])+','+str(json.dumps(distanceWithDayAll[i]))+'\n')
z.close()

y = open('/home/ytwen/observationData_follower_one/fs/sumData_distance','wb')
